  ...
  `;
    ...
    const row =
      recovery.action === "completed"
        ? (await pool.query(fullQuery, [jobId]).catch(() => pool.query(basicQuery, [jobId])))
            .rows[0]
        : result.rows[0];
...
router.post(
  ...
  async (req, res) => {
    ...
    if (job.status !== "processing") {
      return res
        .status(409)
        .json({ message: `Job em status "${job.status}" — não é possível concluir.` });
    }
...
function startImportWorker({ jobId, operator, userId, files }) {
  ...
  worker.on("message", async (msg) => {
    if (msg?.ok) {
      const recovery = await recoverStuckJob(pool, jobId);
      if (recovery.action !== "completed") {
        const row = await pool.query(`SELECT status FROM import_jobs WHERE id = $1`, [jobId]);
